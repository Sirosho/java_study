...
public class LoginUser {
    ...
    private String account;// 가입시 등록한 계정
    ...
    public String authenticate(String inputAccount, String inputPassword)
            throws Exception{ // 이 메서드는 에러 가능성이 있는 위험한 메서드다.
                                // 따라서 사용자는 직접 예외처리를 할 것!
                                // 을 알리는 구문 호출자가 예외처리를 직접안해주면 에러발생함

        /*
                예외를 던지다 : 예외가 발생하면 코드의 흐름이 끊김.
                                마치 리턴문 탈출처럼 에러를 활용할 수 있다.
                                다만 에러가 발생하면 프로그램이 튕기기 때문에
                                그에 따른 예외처리는 호출하는 자에게 떠넘긴다.
